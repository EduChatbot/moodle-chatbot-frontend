...
export default function QuizList() {
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/v1/quiz/course/${courseId}/available`, {
        headers: {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/v1/quiz/history?courseId=${courseId}`, {
        headers: {
          'Authorization': `Bearer ${moodleToken}`
        }
      });
  ...
  const handleGenerateQuiz = async (numQuestions = 5) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/api/v1/quiz/generate/${courseId}?num_questions=${numQuestions}`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${moodleToken}`
